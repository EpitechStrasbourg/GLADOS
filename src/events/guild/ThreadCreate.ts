import { ChannelType, Embed, EmbedBuilder, ThreadChannel, type CacheType, type Interaction } from "discord.js"
...
export default async (interaction: ThreadChannel) => {
    ...
    if (similarThreadsLinks.length > 0) {
      const embed = new EmbedBuilder()
          ...
          .setDescription(similarThreadsLinks.map(t => `[${t.title}](${t.link})`).join('\n'))
          .setColor('#00AAFF')
          .setTimestamp();
